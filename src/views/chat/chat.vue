    ...
    <table>
        ...
        <tbody>
            <tr>
                ...
                <td>李白</td>
                <td>
                    <button @click="openModal(1)">Open Modal</button>
                </td>
            </tr>
            <tr>
                <td>2</td>
                <td>张三</td>
                <td>
                    <button @click="openModal(2)">Open Modal</button>
                </td>
            </tr>
        </tbody>
        </table>
...
import { ApiService } from "../../conf/apiServer";
...
export default {
  ...
  data() {
    return {
      ...
      loading:false,
      defualtAvatar:require('../../static/imgs/admin.png'), // 后端没有返回头像默认头像，注意：需要用require请求方式才能动态访问本地文件
      data:[],
      distincData:[], // 消息去重数组
  ...
  methods: {
    ...
    loadMsg(){ // 窗体打开默认加载一页数据，窗体生命周期中只运行一次
      let that = this;
      this.searchForm.offsetMax = this.offsetMax;
      ApiService.getDriverListMsg(this.searchForm).then(res=>{
        if (res.code == 200) {
          res.data.forEach(e => {
            // 标记最大偏移位
    ...
    openModal(id) {
        this.selectedItemId = id; // 保存选中的id
        this.chatVisible = true; // 打开Modal
    },
    sendMsg(){ // 发送消息
      if(!this.form.msg){
      ...
      ApiService.sendOrderMsg(this.form).then(res=>{
        if (res.code == 200) {
   ...
   reloadData(){
    if(this.timerSwitch){
      setTimeout(() => {
        ...
        let that = this;
        ApiService.listMsgByOrder(params).then(res=>{
          if (res.code == 200) {
            res.data.forEach(e => {
